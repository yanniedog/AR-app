#!/usr/bin/env node
/**
 * Apply branch protection on main requiring app CI and bot merge gates.
...
PRs without merge unless the user waives that explicitly.
`);
}
...
function main() {
  ...
  if (r.status === 0) {
    ...
    if (existingContexts.length) {
      console.log(`Preserved ${existingContexts.length} existing check(s); required checks: ${REQUIRED_CHECKS.join(', ')}`);
    }
