...
export function gateFeedbackPlan(prNumber, { skip, waitPass, feedbackPass }) {
  ...
  if (!waitPass) {
    return {
      id: 'feedback-plan',
      pass: true,
      detail: 'Deferred until wait-for-bots exit 0',
      waived: true,
    };
  }
...
export function gateShipCloseoutSubgates(waitGate, feedbackGate) {
  ...
  return {
    ...
    pass,
    detail: pass
      ? 'Bot wait and feedback thread gates passed'
      : 'wait-for-bots and pr:bot-feedback-check must both pass before merge',
    action: pass ? undefined : 'Fix bot wait / thread feedback, then rerun npm run pr:gates:check',
    note: 'Use pr:gates:check for AR-app merge readiness',
  };
}
