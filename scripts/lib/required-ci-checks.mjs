...
export const DEFAULT_REQUIRED_CHECKS = ["mobile-ci", "bot-feedback-gate"];
...
export function combineRequiredCheckState({
  ...
  return {
    values: [...DEFAULT_REQUIRED_CHECKS],
...
/**
 * Evaluate exact required contexts on the current PR head. Missing contexts are
 * pending: GitHub branch protection will not allow a merge until they exist.
 */
export function evaluateRequiredCheckState({
  ...
  const activeNames = [...new Set((requiredNames || []).filter(Boolean))].filter(
    (name) => !ignored.has(normalizedName(name)),
  );
  const observations = [
    ...(prChecks || []),
    ...(headCheckRuns || []),
...
function prRequiredChecks(prNumber) {
  ...
  // gh uses exit 8 for pending checks but still emits usable JSON.
  if (result.status === 0 || result.status === 8) {
    try {
      ...
      return {
        ok: false,
        error: `Invalid JSON from gh pr checks: ${error.message}`,
      };
    }
  }
  const error =
    (result.stderr || result.stdout || `gh exit ${result.status}`).trim();
  if (
    /no required checks reported/i.test(error) ||
    /no checks reported/i.test(error)
  ) {
    return { ok: true, data: [] };
  }
...
/**
 * Read required policy plus check-runs/statuses bound to the exact PR head.
 * This catches workflow_dispatch checks, which `gh pr checks` can omit.
 */
export function fetchRequiredCheckState({
  ...
  const policy = policyForBranch(repo, baseRefName);
