...
assert.equal(isReviewablePath('.cursor/PR_REVIEW_PROMPT.md'), true);
...
assert.match(workflow, /QWEN_CONTEXT_TOKENS:\s*"32768"/);
assert.match(workflow, /QWEN_VALIDATOR_CONTEXT_TOKENS:\s*"8192"/);
...
assert.match(reviewer, /DEFAULT_MODEL = 'qwen2\.5-coder-review:1\.5b'/);
assert.match(reviewer, /DEFAULT_VALIDATOR_MODEL = 'qwen2\.5-coder-review:7b'/);
...
assert.match(reviewer, /DEFAULT_CHUNK_MAX = 24_000/);
assert.match(reviewer, /\/api\/chat/);
assert.doesNotMatch(reviewer, /\/chat\/completions/);
assert.match(reviewer, /stream:\s*true/);
assert.match(reviewer, /response\.body\.getReader\(\)/);
assert.match(reviewer, /invalid streamed JSON/);
assert.match(reviewer, /num_predict:\s*384/);
assert.match(reviewer, /format:\s*REVIEW_FORMAT/);
assert.match(reviewer, /think:\s*false/);
assert.match(reviewer, /rejected_findings:\s*rejectedFindings/);
assert.match(reviewer, /validation_calls:\s*validationCalls/);
assert.match(reviewer, /num_ctx:\s*contextTokens/);
assert.match(reviewer, /Buffer\.byteLength/);
assert.match(reviewer, /conservativeInputByteBudget/);
assert.match(reviewer, /streamCompleted/);
assert.match(reviewer, /terminal done=true envelope/);
assert.match(reviewer, /'--text'/);
assert.match(reviewer, /cannot fit the complete changed line/);
assert.match(reviewer, /candidate validation failed/);
