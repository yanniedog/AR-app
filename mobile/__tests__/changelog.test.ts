...
describe('changelog', () => {
  it('changelogSummaryUrl points at rolling release asset', () => {
    expect(changelogSummaryUrl('yanniedog/AR-app', 'app-apk-latest')).toBe(
      'https://github.com/yanniedog/AR-app/releases/download/app-apk-latest/changelog-summary.json',
    );
  });
  ...
  it('changelogSummaryUrlFromManifestUrl derives summary URL from manifest URL', () => {
    ...
    expect(
      changelogSummaryUrlFromManifestUrl(
        'https://github.com/yanniedog/AR-app/releases/download/app-apk-latest/app-apk-latest.json?cache=1',
      ),
    ).toBe(
      'https://github.com/yanniedog/AR-app/releases/download/app-apk-latest/changelog-summary.json',
    );
  ...
  it('selectCumulativeChangelogs returns skipped versions only', () => {
    const manifest = { schema_version: 1, repo: 'yanniedog/AR-app', versions };
