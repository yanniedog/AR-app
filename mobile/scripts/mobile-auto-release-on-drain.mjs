...
const repo =
  (repoArgIdx >= 0
    ? process.argv[repoArgIdx + 1]
    : process.env.GITHUB_REPOSITORY
  )?.trim() || "yanniedog/AR-app";
...
export function checkedGhOutput(result, args = []) {
  if (result.error) {
    throw new Error(
      `gh ${args.join(" ")} failed to execute: ${result.error.message}`,
    );
  }
  if (result.status !== 0) {
    throw new Error(
...
function ghTry(args) {
  ...
  if (res.error) {
    throw new Error(
      `gh ${args.join(" ")} failed to execute: ${res.error.message}`,
    );
  }
  return {
    ok: res.status === 0,
...
function dispatchApkBuild(version) {
  if (dryRun) {
    console.log(
      `mobile-auto-release-on-drain: dry-run — would dispatch mobile-android-apk for v${version}`,
    );
    return;
  }
  gh([
  ...
  console.log(
    `mobile-auto-release-on-drain: dispatched mobile-android-apk for v${version} on main`,
  );
}
...
export function ensureApkForMainHead({
  ...
  if (releaseExists(version)) {
    console.log(
      `mobile-auto-release-on-drain: app-v${version} already published — no APK dispatch`,
    );
    return false;
  }
  if (buildInFlight()) {
    console.log(
      "mobile-auto-release-on-drain: mobile-android-apk already queued/in-progress — no APK dispatch",
    );
    return false;
  }
  dispatch(version);
...
export async function waitForQueueDrain({
  ...
  for (let attempt = 1; attempt <= POLL_ATTEMPTS; attempt++) {
    ...
    if (attempt === POLL_ATTEMPTS) {
      console.log(
        "mobile-auto-release-on-drain: queue not drained after polling — skip release",
      );
      return open;
    }
...
function publishViaPullRequest(next, message) {
  ...
  if (!prNumber)
    throw new Error(
      `mobile-auto-release-on-drain: could not parse PR number from ${prUrl}`,
    );

  dispatchRequiredPrChecks(prNumber, branchName);
  enableAutoMerge(prNumber);
  console.log(
    `mobile-auto-release-on-drain: opened fallback PR #${prNumber} with auto-merge (${prUrl})`,
  );
...
async function main() {
  ...
  if (dryRun)
    console.log(
      "mobile-auto-release-on-drain: dry-run — skipping open PR count (assume drained)",
    );
  ...
  console.log(
    `mobile-auto-release-on-drain: queue drained — bump ${current} → ${next}`,
  );
  ...
  if (dryRun) {
    console.log(
      `mobile-auto-release-on-drain: dry-run — would open an auto-release PR for v${next}`,
    );
    process.exit(0);
  }
  ...
  if (bump.status !== 0)
    throw new Error(
      (bump.stderr || bump.stdout || "bump-app-patch-version failed").trim(),
    );
  ...
  if (ensureEntry.status !== 0) {
    throw new Error(
      (
        ensureEntry.stderr ||
        ensureEntry.stdout ||
        "ensure-changelog-entry failed"
      ).trim(),
    );
  }

  const buildManifest = spawnSync(
    "node",
    ["scripts/build-changelog-manifest.mjs", "--repo", repo],
    {
      encoding: "utf8",
      cwd: mobileDir,
    },
  ...
  if (buildManifest.status !== 0) {
    throw new Error(
      (
        buildManifest.stderr ||
        buildManifest.stdout ||
        "build-changelog-manifest failed"
      ).trim(),
    );
  }
