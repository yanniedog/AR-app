...
import { validateActivityAsset } from './validation';
import { operationBudget } from '../monetaryContracts/validation';
import { mortgageSnapshotBudget } from '../mortgageContracts/snapshotBudget';
import { detailsDisplayIdentity } from '../detailsCatalogue';
import type { ActivitySubject, ActivityApproval } from './types';
import type { MonetaryDescriptor, MonetaryRouting } from '../monetaryContracts/types';
export type ActivityContext = EligibilityContext;
export type ActivityTarget = EligibilityTarget;
export interface ActivitySelection { readonly subject: ActivitySubject; readonly approval: ActivityApproval; readonly edition: string }
interface Binding { context: ActivityContext; target: ActivityTarget; manifestSha256: string; contentSha256: string; indexSha256: string; shardSha256: string; assetSha256: string }
const bindings = new WeakMap<ActivitySelection, Binding>();
function contextCheck(c: ActivityContext, target: ActivityTarget) {
  const { manifest: m, core, details } = c;
  if (!m || !core || !details || m.repo !== PAYLOAD_REPO || !m.payload_revision || !verifiedCoreContents(c.coreIntegrity) || verifiedCatalogueDetails(core, c.coreIntegrity, m, details) !== details || !Object.hasOwn(details.products, target.productKey) || details.products[target.productKey] !== target.detail || target.kind === 'rate_variant' && (target.section !== 'Savings' || !core.sections.Savings.rates.includes(target.row) || target.row.product_key !== target.productKey)) throw new Error('Activity product publication is not verified');
  if (detailsDisplayIdentity(target.detail).productCategory !== 'TRANS_AND_SAVINGS_ACCOUNTS') throw new Error('Activity product category unsupported');
  return { m, core, details };
}
function route(c: ActivityContext, target: ActivityTarget) {
  const { m } = contextCheck(c, target);
  if (Object.entries(m.files).some(([key, value]) => /^(monetary|executable)_v[34]_/.test(key) || /^(monetary|executable)_v[34]_/.test(value.name))) throw new Error('Misplaced monetary descriptors');
  if (!Object.hasOwn(m, 'executable_v4')) return null;
  if (utf8ToBytes(JSON.stringify(m)).length > 64 * 1024) throw new Error('Monetary manifest size limit'); assertActivityWire(m.executable_v4, 'namespace');
  ...
  for (const [capability, inventory] of Object.entries(m.executable_v4!.capabilities)) {
    descriptor(inventory!.index, `monetary_v4_${capability}_index`); Object.entries(inventory!.shards).forEach(([key, d]) => descriptor(d, key));
  }
  if (bytes > 8 * 1024 * 1024) throw new Error('Shared publication transfer budget exceeded');
  return m.executable_v4!.capabilities.savings_activity_calculation ?? null;
}
function routingCheck(r: MonetaryRouting, c: ActivityContext, target: ActivityTarget) {
  // Called synchronously only after the boundary has verified the adopted context.
  const m = c.manifest!;
  if (r.productKey !== target.productKey || r.runDate !== m.run_date || r.coreAssetSha256 !== m.files.core.sha256 || r.detailsAssetSha256 !== m.files.details.sha256 || r.sourceGenerationId !== m.source_observation?.generation_id || r.exportContractSha256 !== m.source_observation?.contract_digest || r.productRecordSha256 !== hashText(canonical(target.detail))) throw new Error('Activity routing source association mismatch');
}
async function acquire(c: ActivityContext, d: MonetaryDescriptor, budget: ReturnType<typeof mortgageSnapshotBudget>) {
...
function envelope(v: any, c: ActivityContext, kind: 'index' | 'shard') { assertActivityWire(v, kind); if (v.run_date !== c.manifest!.run_date || v.core_asset_sha256 !== c.manifest!.files.core.sha256 || v.details_asset_sha256 !== c.manifest!.files.details.sha256) throw new Error('Monetary envelope edition mismatch'); }
/** Only activity assets are fetched. Historical raw members remain producer-private. */
export async function loadActivitySelections(c: ActivityContext, target: ActivityTarget): Promise<ActivitySelection[]> {
  ...
  if (!Object.hasOwn(shard.products, target.productKey)) throw new Error('Activity product absent from its shard');
  const assets = Object.entries(shard.products).map(([key, raw]) => {
    const asset = validateActivityAsset(raw); if (key !== asset.productKey || index.products[key] !== shardKey || !Object.hasOwn(c.details!.products, key)) throw new Error('Monetary product map mismatch');
    routingCheck(asset.routing, c, { kind: 'product', productKey: key, detail: c.details!.products[key] }); return asset;
  });
  operationBudget(assets.flatMap(a => a.subjects.map(e => e.subject)));
  if (hashText(canonical(c.manifest)) !== manifestSha256 || c.core !== adoptedCore) throw new Error('Activity publication changed');
  const asset = assets.find(a => a.productKey === target.productKey)!;
  return asset.subjects.map(({ subject, approval }) => {
...
export function assertActivitySelection(selection: ActivitySelection, c: ActivityContext, target: ActivityTarget) {
  const b = bindings.get(selection); contextCheck(c, target);
  if (!b || b.context.manifest !== c.manifest || b.context.core !== c.core || b.context.coreIntegrity !== c.coreIntegrity || b.context.details !== c.details || b.target.kind !== target.kind || b.target.productKey !== target.productKey || b.target.detail !== target.detail || target.kind === 'rate_variant' && (b.target.kind !== 'rate_variant' || b.target.row !== target.row || b.target.section !== target.section) || hashText(canonical(c.manifest)) !== b.manifestSha256 || hashText(canonical(selection)) !== b.contentSha256) throw new Error('Activity selection is stale or unverified'); routingCheck(selection.subject.routing, c, target);
