...
export function boundedInputs(value:unknown){if(utf8ToBytes(canonical(value)).length>512*1024)throw Error('Receipt inputs exceed limit.');let nodes=0;function visit(v:any,depth:number){if(++nodes>8192||depth>16||typeof v==='string'&&v.length>16384)throw Error('Receipt input structure exceeds limits.');if(v&&typeof v==='object')Object.values(v).forEach(x=>visit(x,depth+1));}visit(value,0);}
