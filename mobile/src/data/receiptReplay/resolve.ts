import { loadActivitySelections,type ActivitySelection } from '../activityContracts/transport';
...
export interface ReplayOption {item:ReplayItem;selection:ApprovedSelection|EligibilitySelection|SavingsSelection|ActivitySelection|MortgageSelection;target:EligibilityTarget|RateRow;id:string;label:string;changed:boolean;context:EligibilityContext}
...
export async function resolveReplayItem(item:ReplayItem,c:EligibilityContext,rowChoice?:RateRow):Promise<ReplayOption[]>{
 ...
 return selections.filter(s=>('template'in s?s.template:s.subject).evaluatorVersion===(item.kind==='savings_activity_calculation'?'product-terms-engine-v9':'product-terms-engine-v8')).map(selection=>{const s='template'in selection?selection.template:selection.subject,scope='scope'in s?s.scope:null;return {item,selection,target,id:s.id,label:scope?Object.values(scope).filter(v=>typeof v==='string').join(' · '):`${(s as any).cohortKey} · ${(s as any).tierKey} · ${(s as any).packageKey}`,changed:before!==item.edition||s.id!==item.sourceId||!!row&&hashText(canonical(row))!==item.target.rowSha256,context:c};});
}
