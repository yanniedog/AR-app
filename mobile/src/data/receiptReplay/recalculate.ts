import { calculateSavingsActivityPeriod } from '../activityContracts/adapter';
...
export function recalculateReceipt(document:ReplayDocument,c:ReplayOption['context'],choices:ReplayChoice[]){
 ...
 else {const x=choices[0],o=x.option,s:any=o.selection,t:any=o.target,p=profiles[0];result=o.item.kind==='td'?privateTdExport(calculateDeposit(s,c,t,x.inputs,p),[tdReopenItem(s,c,t,x.inputs,p)]):o.item.kind==='eligibility_only'?evaluateEligibilitySelection(s,c,t,x.inputs,p):o.item.kind==='savings_activity_calculation'?calculateSavingsActivityPeriod(s,c,t,x.inputs,p):o.item.kind==='savings_calculation'?calculateSavingsPeriod(s,c,t,x.inputs,p):calculateMortgagePeriod(s,c,t,x.inputs,p);}
 return result;
}
