...
import { validateMonetaryAsset, operationBudget } from './validation';
import type { MonetaryDescriptor, SavingsSubject, SavingsApproval, MonetaryRouting } from './types';
export type SavingsContext = EligibilityContext;
export type SavingsTarget = EligibilityTarget;
export interface SavingsSelection { readonly subject: SavingsSubject; readonly approval: SavingsApproval; readonly edition: string }
interface Binding { context: SavingsContext; target: SavingsTarget; manifestSha256: string; contentSha256: string; indexSha256: string; shardSha256: string; assetSha256: string }
const bindings = new WeakMap<SavingsSelection, Binding>();
function contextCheck(c: SavingsContext, target: SavingsTarget) {
  const { manifest: m, core, details } = c;
  if (!m || !core || !details || m.repo !== PAYLOAD_REPO || !m.payload_revision || !verifiedCoreContents(c.coreIntegrity) || verifiedCatalogueDetails(core, c.coreIntegrity, m, details) !== details || !Object.hasOwn(details.products, target.productKey) || details.products[target.productKey] !== target.detail || target.kind === 'rate_variant' && (target.section !== 'Savings' || !core.sections.Savings.rates.includes(target.row) || target.row.product_key !== target.productKey)) throw new Error('Savings product publication is not verified');
  return { m, core, details };
}
function route(c: SavingsContext, target: SavingsTarget) {
  ...
  return m.executable_v3!.capabilities.savings_calculation ?? null;
}
function routingCheck(r: MonetaryRouting, c: SavingsContext, target: SavingsTarget) {
  // Called synchronously only after the boundary has verified the adopted context.
  const m = c.manifest!;
  if (r.productKey !== target.productKey || r.runDate !== m.run_date || r.coreAssetSha256 !== m.files.core.sha256 || r.detailsAssetSha256 !== m.files.details.sha256 || r.sourceGenerationId !== m.source_observation?.generation_id || r.exportContractSha256 !== m.source_observation?.contract_digest || r.productRecordSha256 !== hashText(canonical(target.detail))) throw new Error('Savings routing source association mismatch');
}
async function acquire(c: SavingsContext, d: MonetaryDescriptor) {
  return JSON.parse(await downloadInflate(`https://github.com/${PAYLOAD_REPO}/releases/download/${c.manifest!.tag}/${d.name}`, d.sha256, { fileName: d.name, expectedBytes: d.bytes, requireExactBytes: true, maxCompressedBytes: 512 * 1024, maxInflatedBytes: 512 * 1024, allowEncrypted: false }));
}
function envelope(v: any, c: SavingsContext, kind: 'index' | 'shard') { assertMonetaryWire(v, kind); if (v.run_date !== c.manifest!.run_date || v.core_asset_sha256 !== c.manifest!.files.core.sha256 || v.details_asset_sha256 !== c.manifest!.files.details.sha256) throw new Error('Monetary envelope edition mismatch'); }
/** Only savings assets are fetched. Historical raw members remain producer-private. */
export async function loadSavingsSelections(c: SavingsContext, target: SavingsTarget): Promise<SavingsSelection[]> {
  const inventory = route(c, target); if (!inventory) return [];
  const adoptedCore = c.core, manifestSha256 = hashText(canonical(c.manifest)), index = await acquire(c, inventory.index); envelope(index, c, 'index'); contextCheck(c, target);
  ...
  const shardKey = index.products[target.productKey], d = inventory.shards[shardKey], shard = await acquire(c, d); envelope(shard, c, 'shard'); contextCheck(c, target);
  if (!Object.hasOwn(shard.products, target.productKey)) throw new Error('Savings product absent from its shard');
  const assets = Object.entries(shard.products).map(([key, raw]) => {
    const asset = validateMonetaryAsset(raw); if (key !== asset.productKey || index.products[key] !== shardKey || !Object.hasOwn(c.details!.products, key)) throw new Error('Monetary product map mismatch');
  ...
  if (hashText(canonical(c.manifest)) !== manifestSha256 || c.core !== adoptedCore) throw new Error('Savings publication changed');
...
export function assertSavingsSelection(selection: SavingsSelection, c: SavingsContext, target: SavingsTarget) {
  const b = bindings.get(selection); contextCheck(c, target);
  if (!b || b.context.manifest !== c.manifest || b.context.core !== c.core || b.context.coreIntegrity !== c.coreIntegrity || b.context.details !== c.details || b.target.kind !== target.kind || b.target.productKey !== target.productKey || b.target.detail !== target.detail || target.kind === 'rate_variant' && (b.target.kind !== 'rate_variant' || b.target.row !== target.row || b.target.section !== target.section) || hashText(canonical(c.manifest)) !== b.manifestSha256 || hashText(canonical(selection)) !== b.contentSha256) throw new Error('Savings selection is stale or unverified'); routingCheck(selection.subject.routing, c, target);
