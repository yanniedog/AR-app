...
function templatesFor(inputs: DeepAuditDerivedInputs): StepTemplate[] {
  ...
  scenario(templates, 'route.calculator', {
    ...
    skipExplanation: 'Existing encrypted inputs are read only; no balance, rate, term or property value is entered.',
    unsafeActionsExcluded: ['financial-input.edit'],
  }, [
    { depth: 0, semanticActionId: 'calculator.open' },
    { depth: 1, semanticActionId: 'calculator.section.next', parameters: { section: section ?? null }, stateImpact: 'local-only' },
  ...
  scenario(templates, 'route.projections', {
    ...
    skipExplanation: 'Graph actions run only when the existing scenario is valid; financial inputs are never fabricated.',
    unsafeActionsExcluded: ['financial-input.edit'],
  }, [
