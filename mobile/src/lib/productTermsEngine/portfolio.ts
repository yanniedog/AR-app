...
import { PORTFOLIO_EVALUATOR_VERSION, FIXED_MATURITY_EVALUATOR_VERSION, EVALUATOR_VERSION, type CalculationReceipt, type LedgerEntry } from './types';
...
export function calculatePortfolio(input: PortfolioInput, budget = new EvaluationBudget()): PortfolioReceipt {
  ...
  try {
    ...
    if (input.transfers.some(t => t.status === 'projected')) {
      ...
      if (a && typeof a.id === 'string' && /^[A-Za-z0-9_.:-]{1,180}$/.test(a.id) && a.acknowledged === true && a.transfersSha256 === sha && input.accounts.every(a => [EVALUATOR_VERSION, FIXED_MATURITY_EVALUATOR_VERSION, PORTFOLIO_EVALUATOR_VERSION].includes(a.contract.evaluatorVersion as typeof EVALUATOR_VERSION))) {
