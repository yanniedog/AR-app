...
export function portfolioAuthorities(input: PortfolioInput): Map<string, AccountAuthority> {
  ...
  for (const p of packages) {
    ...
    for (const accountId of p.memberAccountIds) {
      ...
      if (account.contract.evaluatorVersion !== EVALUATOR_VERSION || !Array.isArray(p.evidenceIds) || !p.evidenceIds.length || p.evidenceIds.some(id => !account.contract.evidence.some(e => e.id === id))) throw new Error('portfolio_package_evidence_missing');
  ...
  for (const route of routes) {
    ...
    if (!account?.contract.tdLifecycle || account.contract.evaluatorVersion !== EVALUATOR_VERSION || routed.has(route.accountId) || route.generalFees !== 'external_only' || route.lifecycleOccurrenceId !== 'td:break-fee' ||
